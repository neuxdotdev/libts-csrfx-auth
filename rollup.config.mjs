...
const getMinificationPlugins = (mapFilename = 'index.[format].js.map') => {
	const terserOptions = {
		ecma: 2022,
		module: true,
		compress: {
...
const postBuildHook = (outputFile) => ({
	name: 'post-build-hook',
	async writeBundle() {
		if (process.env.GENERATE_PKG === 'true') {
			const pkgCopy = { ...packageJson }
			delete pkgCopy.devDependencies
			delete pkgCopy.scripts
			pkgCopy.main = './index.cjs'
			pkgCopy.module = './index.mjs'
			pkgCopy.types = './index.d.ts'
			writeFileSync(
...
const createLibConfig = ({ input, outputDir, minify = false, formats = ['esm', 'cjs'] }) => {
	...
	const plugins = [
		...
		postBuildHook(resolve(absoluteOutputDir, 'index.mjs')),
	]
	if (analyze) {
		plugins.push(
			visualizer({
